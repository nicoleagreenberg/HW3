...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://nicoleackermangreenberg@localhost:5432/nicoleaghw3"
## Provided:
...
class TwitterForm(FlaskForm):
	...
	def validate_username(self, field):
		if "@" in field.data:
			raise ValidationError("Your username may not contain an "@" symbol")

	def validate_display_name(self, field):
		display_name_input = field.data.split()
		if len(display_name_input)<2:
...
def index():
	...
	tweet_objects = Tweet.query.all() #this returns the post objects
	...
	## Find out if there's already a user with the entered username
		...
		user = User.query.filter_by(username=username).first() #if something is returned, user is True, returns None if there isn't anything in the database
	...
	## If there already exists a tweet in the database with this text and this user id (the id of that user variable above...) 
		text = form.text.data
		existingtweet = Tweet.query.filter_by(text=text, user_id=user.id).first()

	## Then flash a message about the tweet already existing

		if existingtweet:
			flash("This tweet already exists, we're going to redirect you to the list of all tweets instead of posting it.")
	...
	## Create a new tweet object with the text and user id
		newtweet = Tweet(text=text, user_id=user.id)
		db.session.add(newtweet)
		db.session.commit()
	## And add it to the database
	## Flash a message about a tweet being successfully added
		flash("Tweet has been successfully tweeted! It's now in the database.")
...
def see_all_tweets():
	...
	for t in all_tweets:
		...
		tweets_users.append((t.text, user.username))
